  ...
  <div class="m-10">
    ...
    >
      <div class="flex flex-col flex-col-reverse w-80">
        <div
          class="flex w-full flex flex-row-reverse bg-black text-white overflow-x-auto"
        >
          <span
            v-for="(st, index) in states"
            :key="st.id"
            class="text-white bg-black flex first:mr-auto min-w-max"
          >
            {{ st }}
            <icon
              size="20"
      ...
      <div>
        <ul
          v-if="noFilteredState"
          class="bg-black p-1 w-full top-11 rounded-md"
        >
